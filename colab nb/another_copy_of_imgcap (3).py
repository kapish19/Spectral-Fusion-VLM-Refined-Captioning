...
# ----------------------------
# VLM Refinement: LLaVARefiner
# ----------------------------

# Lightweight NLP deps (NLTK)
import nltk
try:
...
class LLaVARefiner:
    """
    Lightweight VLM refinement wrapper.
    - Default behavior: rule-based grounding + clause pruning using YOLO detections.
    - Optional: plug your LLaVA/VLM inference function into `self._call_llm` and set use_llm=True.
    """
    def __init__(self, yolo_obj, use_llm=False, llm_infer_callable=None, detection_threshold=0.25):
        """
        Args:
            yolo_obj: the instantiated YOLO object used in your pipeline.
            use_llm: whether to call an external LLaVA/VLM model (if available).
            llm_infer_callable: optional callable fn(image, det_list, draft) -> refined_caption
            detection_threshold: confidence threshold for YOLO detections
        """
        self.yolo = yolo_obj
        self.use_llm = use_llm
        self.llm_infer_callable = llm_infer_callable
        self.det_thr = detection_threshold

        # try to fetch the COCO class names if available on the yolo object
        self.coco_names = None
        try:
            # yolov4.tf YOLO object stores names after parse_names
            self.coco_names = self.yolo.config.names
        except Exception:
            self.coco_names = None

    def _get_yolo_labels(self, image_path):
        """Return a set of detected label strings for the given image path."""
        try:
            import cv2
            frame = cv2.imread(image_path)
            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            preds = self.yolo.predict(frame, prob_thresh=self.det_thr)
            # preds -> list of boxes; each box is something like [x,y,w,h,class,score]
            labels = []
            for b in preds:
                # some yolov4 versions return numpy arrays, some lists
                try:
                    cls_idx = int(b[4])
                except Exception:
                ...
                try:
                    ...
                    score = None
                # map cls_idx -> name if possible; else use numeric
                if cls_idx is not None and self.coco_names is not None:
                    name = self.coco_names.get(cls_idx, None) if isinstance(self.coco_names, dict) else None
                    if name is None:
                        # try list-like names
                        try:
                            name = self.coco_names[cls_idx]
                        except Exception:
                            name = str(cls_idx)
                    ...
                    name = "object"
                # Only keep detections above threshold (YOLO predict already thresholded)
                labels.append(name.lower())
            return set(labels)
        except Exception as e:
            # If YOLO fails, return empty set
            print("⚠️ LLaVARefiner: YOLO label extraction failed:", e)
            return set()

    def _rule_refine(self, image_path, draft):
        """
        Rule-based refinement: remove clauses with no grounded nouns.
        - Tokenizes draft, extracts noun phrases (simple noun detection via POS tags),
          and keeps only clauses that contain a grounded noun.
        - Adds a simple relation heuristic when two related classes co-occur (e.g., person + bicycle -> 'riding').
        """
        detected = self._get_yolo_labels(image_path)

        if not draft or len(draft.strip()) == 0:
            return draft, {}

        # basic clause splitting heuristics (commas / ' and ' / ' with ')
        clauses = []
        # keep original separators to reconstruct somewhat natural text
        raw_clauses = []
        # split by comma first
        for c in draft.split(','):
            # further split by ' and ' or ' with '
            subparts = []
            for s in c.split(' and '):
                for t in s.split(' with '):
                    t = t.strip()
                    if t:
                        subparts.append(t)
            for s in subparts:
                raw_clauses.append(s.strip())

        # if splitting found nothing, use the whole draft
        if not raw_clauses:
            raw_clauses = [draft.strip()]

        kept_clauses = []
        grounding_map = {}  # word -> grounded True/False
        for clause in raw_clauses:
            tokens = word_tokenize(clause)
            tags = pos_tag(tokens)
            # find nouns (NN, NNS, NNP, NNPS)
            nouns = [tok.lower() for tok, tag in tags if tag.startswith('NN')]
            # is clause grounded if any noun matches detected labels (substring match allowed)
            clause_grounded = False
            for n in nouns:
                grounded = False
                for lab in detected:
                    if n in lab or lab in n or n.rstrip('s') in lab or lab in n.rstrip('s'):
                        grounded = True
                        break
                grounding_map[n] = grounded
                if grounded:
                    clause_grounded = True
            # if clause has no noun, keep if it contains verbs/ adjectives (short descriptive fragment)
            if clause_grounded or (len(nouns) == 0):
                kept_clauses.append(clause)
        # If no clauses survived (overly strict), fallback to keep the original draft
        if not kept_clauses:
            refined = draft.strip()
        else:
            # Join kept clauses into a single sentence (simple join)
            refined = '; '.join(kept_clauses).strip()
            # ensure punctuation
            if refined and refined[-1] not in '.!?':
                refined = refined + '.'

        # Heuristic relation enrichment: if 'person' & 'bicycle' detected but 'ride' not in text, add "riding" relation
        rv = refined.lower()
        if ('person' in detected or 'man' in detected or 'woman' in detected) and ('bicycle' in detected or 'bike' in detected) and 'ride' not in rv and 'riding' not in rv:
            refined = refined.rstrip('.')
            refined += ' riding a bicycle.'
        # Another simple example: person + dog -> 'playing' if not present
        if ('person' in detected or 'man' in detected or 'woman' in detected) and ('dog' in detected) and 'play' not in rv and 'playing' not in rv:
            refined = refined.rstrip('.')
            refined += ' playing with a dog.'

        return refined, grounding_map

    def _call_llm(self, image_path, detected_labels, draft):
        """
        Placeholder for calling an external LLaVA / VLM. If you have a callable
        llm_infer_callable (provided at init), call it here and return its result.
        """
        if self.llm_infer_callable is not None:
            try:
                return self.llm_infer_callable(image_path, list(detected_labels), draft)
            except Exception as e:
                print("⚠️ LLaVARefiner: llm_infer_callable failed:", e)
                # fallback to rules
        return None

    def refine(self, image_path, draft_caption):
        """
        Public method to refine a draft caption.
        Returns: refined_caption (str), grounding_map (dict)
        """
        # Step 1: Get detected labels (for verification)
        detected = self._get_yolo_labels(image_path)

        # Step 2: If user wants true LLaVA and provided callable, prefer that
        if self.use_llm:
            llm_out = self._call_llm(image_path, detected, draft_caption)
            if llm_out is not None:
                # Optionally, you can add a verification pass after LLM using detected labels
                return llm_out, {}  # we don't have token-level grounding map from LLM here
            # else fall through to rule-based

        # Step 3: Rule-based refinement
        refined, grounding_map = self._rule_refine(image_path, draft_caption)

        return refined, grounding_map

# ----------------------------
# Instantiate the refiner (place after YOLO is built)
# ----------------------------
# Make sure this lives where `yolo` is available (after your existing YOLO init).
vlm_refiner = LLaVARefiner(yolo_obj=yolo, use_llm=False)
# If you want to plug a callable that performs LLaVA inference, do:
# vlm_refiner = LLaVARefiner(yolo_obj=yolo, use_llm=True, llm_infer_callable=my_llava_fn)

# ----------------------------
# Integration into evaluate_image / evaluate pipeline
# ----------------------------
# Find your evaluate_image(image_path) function near the bottom of the script.
# After you compute `predicted_caption` (the draft from GRU), call the refiner:
#
#   draft_caption = predicted_caption
#   refined_caption, grounding = vlm_refiner.refine(img_path, draft_caption)
#   results.append({"image_id": ..., "predicted_caption": refined_caption})
#
# Below is an example patch to replace part of your evaluation loop.

# Example: if you have this block near the end of your script:
...
# --- Replace the final inference loop with this safe integration ---
# (scroll to where you evaluate `test_images` and append to results)
new_results = []
for img_path in tqdm(test_images):
    try:
        if not os.path.exists(img_path + "_" + feature_extraction_model + ".npy"):
            continue
        # draft generation (your existing function)
        draft_caption = evaluate_image(img_path)
        # some of your evaluate_image returns a string, some return tuple; ensure string
        if isinstance(draft_caption, tuple) or isinstance(draft_caption, list):
            # if a list of tokens -> join
            if isinstance(draft_caption, list):
                draft_caption = ' '.join(draft_caption)
            else:
                draft_caption = draft_caption[0] if len(draft_caption) > 0 else ""
        draft_caption = draft_caption.replace('<start>', '').replace('<end>', '').strip()

        # VLM refinement (rule-based or real LLaVA if configured)
        refined_caption, grounding_map = vlm_refiner.refine(img_path, draft_caption)
        ...
        print("⚠️ Skipping image due to error:", img_path, e)
        continue

# You can save new_results to JSON similar to your earlier code
import json
with open(os.path.join(save_dir, "results_refined.json"), "w") as f:
    json.dump(new_results, f, indent=2)

print(f"✅ Saved {len(new_results)} refined predictions to {os.path.join(save_dir, 'results_refined.json')}")
